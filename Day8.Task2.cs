...
for (int i = 0; i < 1000000; i++)
{
    ...
    if (connectedBoxes.Count is 1 && connectedBoxes.First().SelectMany(c => new List<(int x, int y, int z)>() { c.From, c.To }).Distinct().Count() == lines.Length)
    {
...
long result = (long)lastConnection.From.x * (long)lastConnection.To.x;
